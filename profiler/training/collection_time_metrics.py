...
def run_model(model_name,session,input_name,output_name,profiling_thread,runs,image_path=None):
    ...
    if (model_name == "alexnet.onnx" or model_name == "resnet.onnx" or model_name == "vgg.onnx" or model_name == "googlenet.onnx"\
        ...
        if model_name != "efficientnet.onnx" and model_name!="efficientnet_part1.onnx":
            ...
            if "part" not in model_name:
                ...
                write_bytes = os.path.getsize("results/" + model_name + "_outputs_" + str(runs) + ".npy")
...
def profiling_onnx_model(runs,device_type,metrics_list,gpu_needed,random_cpu_cores,random_cpu_load):
    ...
    if gpu_needed:
        ...
        gpu_temperature, gpu_utilization_percent, gpu_memory_percent = parse_gpu_stats(gpu_stats)
        metrics = {
                "model_name": model_name,
                "device": device_type,
                "total_memory_usage_percent": memory_usage,
                ...
                "device_cpu_cores": random_cpu_cores,
                "device_load_percent": random_cpu_load,
                "gpu_usage_percent": gpu_utilization_percent,
                "execution_number": runs
            }
    print(metrics)
...
def load_onnx_model(model_path,device_type,model_name,runs,gpu_needed,metrics_list, random_cpu_cores, random_cpu_load, image_path=None):
    ...
    if device_type in devices:
        ...
        profiling_thread = threading.Thread(target=profiling_onnx_model, args=(
        runs, device_type, metrics_list,gpu_needed, random_cpu_cores, random_cpu_load))
        read_bytes, write_bytes = run_model(model_name,session,input_names,output_names,profiling_thread,runs,image_path)
        ...
        tmp["disk_io_read_bytes"] = read_bytes
        tmp["disk_io_write_bytes"] = write_bytes
        tmp["execution_time"] = execution_time
        tmp["start_timestamp"] = str(start_timestamp)
        tmp["end_timestamp"] = str(end_timestamp)
        tmp["total_parameters"] = total_parameters
        #tmp["param_shapes"] = param_shapes
        tmp["conv_layers"] = conv_layers
        tmp["pool_layers"] = pool_layers
        tmp["fully_conn_layers"] = fc_layers
        tmp["filter_details"] = filter_details
...
if __name__ == '__main__':
    ...
    for iteration in range(args.iterations):
        ...
        random_cpu_cores = random.randint(0,3)
        ...
        print('-' * 50)
        if image_path:
            load_onnx_model(model_path, device_type, model_name, runs, gpu_needed, complete_metrics_list,random_cpu_cores, random_cpu_load,string_paths[iteration])
        else:
            load_onnx_model(model_path, device_type, model_name, runs, gpu_needed, complete_metrics_list,random_cpu_cores, random_cpu_load)
        killing_process = end_stresser(process)
        killing_process.wait(timeout=5)
        time.sleep(10)
    #if model_name == "deeplab_part2.onnx":
